...
-- ========== HOMEWORK (NO TELEPORT) + SLEEP + REJOIN ==========
localPlayer.ChildAdded:Connect(function(child)
    if child.Name == "Homework" then
        ...
        -- All homework done: sleep again, then rejoin same server via New Royale
        performDormSleep()
        local placeId = game.PlaceId
        repeat
            replicatedStorage.SceptorTeleport:FireServer("New Royale")
            task.wait(5)
        until game.PlaceId ~= placeId
    end
end)

-- ========== TIME-BASED DORM SLEEP (optional, triggers at 6 AM) ==========
